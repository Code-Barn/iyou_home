...
The application runs a local **WSS** (WebSocket over TLS) server on port 9001 that binds `127.0.0.1:9001` (IPv4 only — all local services standardised to 127.0.0.1). This is started inside the Tauri `.setup()` hook using `tauri::async_runtime::spawn`.
...
**TLS termination.** Modern browsers block `ws://` connections from HTTPS origins (mixed-content security policy). The bridge terminates TLS natively via `tokio_rustls`, accepting connections at `wss://localhost:9001/`. A self-signed certificate is generated at first launch by `rcgen` (SANs: `localhost`, `127.0.0.1`) and cached as PEM files in the app data directory (`bridge_cert.pem`, `bridge_key.pem`). Subsequent launches reuse the cached cert, keeping the trust-anchor stable across restarts.
...
Safari, Chrome, and Brave all require a PNA pre-flight (OPTIONS) before a public HTTPS origin is allowed to connect to a private-network WebSocket. Because `TlsStream` does not support `peek()`, the handler reads the first 4 KiB of decrypted plaintext after the TLS handshake and routes by content:

1. `TlsAcceptor::accept()` performs the TLS handshake, yielding a `TlsStream<TcpStream>`.
2. The handler reads the initial bytes from the decrypted stream into a 4 KiB buffer.
3. If the buffer starts with `b"OPTIONS"` — responds 200 directly over the TLS stream with `Access-Control-Allow-Origin: *`, `Access-Control-Allow-Private-Network: true`, and `Access-Control-Allow-Methods: GET, PUT, POST, OPTIONS`.
...
The self-signed certificate is generated once at first bridge startup and persisted to the app data directory:

| File | Format | Contents |
|---|---|---|
| `bridge_cert.pem` | PEM (X.509) | Self-signed certificate, CN = `iyou-home Local Development Bridge`, SANs = `localhost`, `127.0.0.1` |
| `bridge_key.pem` | PEM (PKCS#8) | Corresponding private key (Ed25519 or RSA, generated by `rcgen`) |

**Generation** (`load_or_generate_certs` in `src-tauri/src/bridge.rs`):
1. Check for existing PEM files; if both exist, load and return immediately (stable trust anchor).
2. If missing, call `rcgen::CertificateParams::new(["localhost", "127.0.0.1"])` → `self_signed(&key_pair)`.
3. Write `cert.pem()` and `key_pair.serialize_pem()` to the app data directory.
4. Re-read through the same PEM parser path to guarantee consistent `CertificateDer`/`PrivateKeyDer` formatting.
5. Construct a `rustls::ServerConfig` with `with_no_client_auth()` + `with_single_cert()`, wrap in `TlsAcceptor`.

**Browser trust.** Because the cert is self-signed, browsers will show a security warning on first `wss://localhost:9001` connection. In development, the user can manually trust the cert (Safari: "Show Certificate" → "Trust"; Chrome: `thisisunsafe` bypass). For production deployment, the Tauri shell could install the cert into the system trust store at first launch.
